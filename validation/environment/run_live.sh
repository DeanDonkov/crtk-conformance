...
if [ "$VER" = "v1" ]; then
  SRC=/rc3/sources/src-v1; AMBF_ARGS="--launch_file $SRC/launch.yaml -l 0,1,3,4,14,15 -p 120 -t 1 --override_max_comm_freq 120 -g false"
else
  SRC=/rc3/sources/src-v2; AMBF_ARGS="--launch_file $SRC/launch.yaml -l 0,1,6,7,8,9 -p 200 -t 1 --override_max_comm_freq 100 --override_min_comm_freq 100 -g false"
fi
...
python3 /rc3/live/live_aux.py --version "$VER" --out "$OUT/live_aux.json" > "$OUT/logs/live_aux.log" 2>&1
