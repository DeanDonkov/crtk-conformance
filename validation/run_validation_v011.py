#!/usr/bin/env python3
"""Validation campaign of crtk-conformance 0.1.1 against crtk-mock (plan: VALIDATION_PLAN_V011.md).

Usage:  python validation/run_validation_v011.py [--quick] [--out validation/v0.1.1/mock] [--only F,S,T,L,P,R,X,M]
...
analyze_v011.py without re-running anything.  Nothing here is a measurement of any platform.
"""
...
def exp_frame(out, quick):
    ...
    for noise in noises_mm:
        for tn in tnorms_mm:
            for ang in angles:
                ...
                log(f"F_id_{k:03d} t={tn}mm th={ang} noise={noise}mm -> {rec['result']['outcome']} |t_hat|={est.get('binding_translation_norm_m',{}).get('mean',float('nan'))*1e3:.3f}mm ({rec['wall_s']:.1f}s)")
                k += 1
    ...
    for j, noise in enumerate(noises_mm):
        cases.append((f"F_exp_{j}", dict(JHU, noise_m=noise / 1000.0, seed=500 + j), jhu_expectation(), {"binding": "JHU", "noise_m": noise / 1000.0, "expect": "JHU", "model_residual_m": 0.0}))
        cases.append((f"F_expwrong_{j}", dict(JHU, noise_m=noise / 1000.0, seed=520 + j), E_ID, {"binding": "JHU", "noise_m": noise / 1000.0, "expect": "identity", "model_residual_m": TOL.spatial_error(0.20, math.radians(150))}))
        cases.append((f"F_disc_{j}", {"bind_translation_m": [0.002, 0, 0], "bind_angle_deg": 5.0, "bind_axis": [0, 0, 1], "noise_m": noise / 1000.0, "seed": 540 + j}, E_DISC, {"binding": "2mm/5deg", "noise_m": noise / 1000.0, "expect": "discover_only"}))
    cases.append(("F_nolocal", {"publish_local": False, "publish_T_b_w": True, "seed": 560}, E_ID, {"binding": "identity", "local": False, "expect": "identity"}))
    for j, od in enumerate([0.1, 0.5, 2.0] if not quick else [0.5]):
        cases.append((f"F_orient_{j}", dict(JHU, orientation_noise_deg=od, noise_m=0.00002, seed=570 + j), jhu_expectation(), {"binding": "JHU", "orientation_noise_deg": od, "noise_m": 0.00002, "expect": "JHU", "model_residual_m": 0.0}))
    for j, sk in enumerate([0.002, 0.030, 0.080] if not quick else [0.080]):
        cases.append((f"F_skew_{j}", dict(JHU, stamp_skew_s=sk, seed=580 + j), jhu_expectation(), {"binding": "JHU", "stamp_skew_s": sk, "expect": "JHU", "pairing_window_s": 0.05}))
...
def exp_scale(out, quick):
    ...
    for noise in noises_mm:
        for s in scales:
            ...
            k += 1
    extra = [
        ("S_noanchor", {"unit_m": 0.1, "seed": 999, "publish_local": False, "publish_T_b_w": True}, E_SI, False, {"s": 0.1, "anchor": False}),
        ("S_disc", {"unit_m": 0.1, "seed": 998, "publish_local": False, "publish_T_b_w": True}, E_DISC, True, {"s": 0.1, "expect": "discover_only"}),
        ("S_unit01", {"unit_m": 0.1, "seed": 997, "publish_local": False, "publish_T_b_w": True}, Expectations.from_dict({"dimensional": {"mode": "si", "expected_unit_m": 0.1}}), True, {"s": 0.1, "expect_unit_m": 0.1, "model_error_m": 0.0}),
    ]
    for j, an in enumerate([0.02, 0.1, 0.5] if not quick else [0.1]):
        extra.append((f"S_anchornoise_{j}", {"unit_m": 0.5, "anchor_noise_m": an / 1000.0, "seed": 980 + j, "publish_local": False, "publish_T_b_w": True}, E_SI, True, {"s": 0.5, "anchor_noise_m": an / 1000.0, "model_error_m": TOL.dimensional_error(0.5)}))
    for j, dp in enumerate([0.1, 0.5]):
        extra.append((f"S_drop_{j}", {"unit_m": 1.0, "drop_prob": dp, "seed": 990 + j, "publish_local": False, "publish_T_b_w": True}, E_SI, True, {"s": 1.0, "drop_prob": dp, "model_error_m": 0.0}))
...
def exp_rate(out, quick):
    ...
    for loop, pub in combos:
        for noise in noises_mm:
            over = {"loop_rate_hz": loop, "publish_rate_hz": pub, "noise_m": noise / 1000.0, "seed": 300 + j}
            ...
            rec["truth"] = {"loop_rate_hz": loop, "publish_rate_hz": pub, "noise_m": noise / 1000.0}
            save(out, f"T_rate_{j:03d}", rec)
            rows = rec["result"]["observations"]["effective_rate"]["per_rate"]
            log(f"T_rate_{j:03d} loop={loop} pub={pub} noise={noise}mm -> {rec['result']['outcome']} " + " ".join(f"{r['command_rate_requested_hz']:.0f}:{r['transitions']}/{r['commands_sent']}={r['observable_rate_hz']:.0f}Hz[{r['status'][:3]}/{r['observation_bounded_by'][:3]}]" for r in rows))
            j += 1
    extra = []
    for k, sp in enumerate([0.05, 0.2] if not quick else [0.05]):
        extra.append((f"T_interp_{k}", {"max_speed_m_s": sp, "publish_rate_hz": 500, "loop_rate_hz": 1000, "seed": 320 + k}, {"max_speed_m_s": sp}))
    for k, dp in enumerate([0.1, 0.5]):
        extra.append((f"T_drop_{k}", {"drop_prob": dp, "seed": 330 + k}, {"drop_prob": dp}))
    for k, jit in enumerate([0.005, 0.02]):
        extra.append((f"T_delay_{k}", {"response_delay_s": 0.05, "response_jitter_s": jit, "seed": 340 + k}, {"response_delay_s": 0.05, "response_jitter_s": jit}))
    for name, over, truth in extra:
        rec = run_probe(lambda a: RateSensitivityProbe(a, TOL, trials=1, gap_max_s=0.3, bisection_steps=2, rates_hz=rates, expectations=E_RATE), "reference", over, expectation=E_RATE)
        rec["truth"] = truth
        save(out, name, rec)
        rows = rec["result"]["observations"]["effective_rate"]["per_rate"]
        log(f"{name} -> {rec['result']['outcome']} " + " ".join(f"{r['command_rate_requested_hz']:.0f}:{r['transitions']}/{r['commands_sent']}[{r['status'][:3]}]" for r in rows))
...
def exp_liveness(out, quick):
    ...
    taus = [0.02, 0.03, 0.05, 0.1, 0.25, 0.5, 1.0] if not quick else [0.03, 0.25]
    ...
    for k, tau in enumerate([0.1, 0.5] if not quick else [0.5]):
        over = {"watchdog_s": tau, "watchdog_mode": "release", "release_drift_m_s": 0.02, "seed": 220 + k, "state_machine": False, "require_enabled": False}
        rec = run_probe(lambda a: RateSensitivityProbe(a, TOL, trials=trials, gap_max_s=1.5, bisection_steps=steps, rates_hz=(100,), expectations=E_HOLD), "reference", over, expectation=E_HOLD)
        rec["truth"] = {"tau_w_s": tau, "mode": "release", "drift_m_s": 0.02, "note": "drift speed is a validation parameter"}
        save(out, f"L_release_{k:03d}", rec)
        log(f"L_release_{k:03d} tau={tau} -> {rec['result']['outcome']} | {rec['result']['observations']['liveness']['finding']}")
    rec = run_probe(lambda a: RateSensitivityProbe(a, TOL, trials=trials, gap_max_s=1.5, bisection_steps=steps, rates_hz=(100,), expectations=E_HOLD), "reference", {"seed": 230}, expectation=E_HOLD)
    rec["truth"] = {"tau_w_s": 0.0, "mode": "none"}
    save(out, "L_hold", rec)
    log(f"L_hold -> {rec['result']['outcome']} | {rec['result']['observations']['liveness']['finding']}")
    rec = run_probe(lambda a: RateSensitivityProbe(a, TOL, trials=trials, gap_max_s=1.5, bisection_steps=steps, rates_hz=(100,), expectations=E_FAULT), "reference", {"watchdog_s": 0.5, "watchdog_mode": "fault", "response_delay_s": 0.3, "seed": 240}, expectation=E_FAULT)
    ...
    rec = run_probe(lambda a: RateSensitivityProbe(a, TOL, trials=trials, gap_max_s=1.5, bisection_steps=steps, rates_hz=(100,), expectations=E_FAULT), "reference", {"watchdog_s": 0.25, "watchdog_mode": "fault", "publish_rate_hz": 20, "seed": 250}, expectation=E_FAULT)
...
def exp_regression(out, quick):
    ...
    cases = {
        "R_noisy": ({"bind_translation_m": [0.002, 0, 0], "noise_m": 0.0005, "seed": 400}, e_all),
        "R_missing_cp": ({"publish_measured_cp": False, "seed": 401}, e_all),
        "R_nolocal": ({"publish_local": False, "seed": 402}, e_all),
        "R_drop_50": ({"drop_prob": 0.5, "seed": 403}, e_all),
        "R_delay_50ms_jitter_20ms": ({"response_delay_s": 0.05, "response_jitter_s": 0.02, "seed": 404}, e_all),
    }
    ...
    for name in PRESETS:
        for label, e in (("discover", E_DISC), ("authored", authored[name])):
            with mock_node(name, {}):
                ...
                rep = build_report(NS, TOL, disc, rs, os.environ.get("ROS_MASTER_URI", ""), expectations=e, parameters={"campaign": "v0.1.1 mock", "preset": name, "label": label})
                a.close()
...
def exp_model_check(out, quick):
    ...
    with mock_node("reference", dict(JHU, unit_m=0.1, seed=502)):
        a = PlatformAdapter(NS, anchor_topic=ANCHOR); a.discover()
        buf = a.subscribe("measured_cp"); truth = a.subscribe("anchor", PoseStamped, full_topic=ANCHOR)
        ensure_enabled(a); a.wait_for(truth, 1.0)
...
def main():
    ...
    ap.add_argument("--out", default=os.path.join(os.path.dirname(os.path.abspath(__file__)), "v0.1.1", "mock"))
    ap.add_argument("--quick", action="store_true")
    ap.add_argument("--only", default="F,S,T,L,P,R,X,M")
    ...
    meta = {
        ...
        "command": " ".join(sys.argv), "plan": "VALIDATION_PLAN_V011.md",
    }
    t0 = time.time()
